...
export default function HUDStrip({ images }:{images:{src:string;alt?:string}[]}){
  ...
  useEffect(()=>{ const el=scroller.current; if(!el) return;
    ...
    return ()=>{ el.removeEventListener('mousedown',onDown); el.removeEventListener('mouseleave',onUp); el.removeEventListener('mouseup',onUp); el.removeEventListener('mousemove',onMove); el.removeEventListener('wheel', onWheel as any); };
  },[]);
  const scrollBy = (dir:number)=>{ const el=scroller.current; if(!el) return; el.scrollBy({ left: dir * el.clientWidth * 0.9, behavior:'smooth' }); };
  return (<section className="py-16 bg-white/5"><div className="container">
    <div className="flex items-end justify-between"><h3 className="text-2xl sm:text-3xl font-semibold">HUD / telemetry</h3>
      <span className="text-sm opacity-70">Use mouse wheel or arrows</span></div>
    <div className="relative mt-6 -mx-4 sm:mx-0">
      ...
      <div ref={scroller} className="overflow-x-auto no-scrollbar px-4 sm:px-0 touch-pan-x cursor-grab select-none">
        <div className="flex gap-4 snap-x snap-mandatory">
          {images.map((img,i)=>(<div key={i} className="snap-center shrink-0 rounded-2xl overflow-hidden border border-white/10 bg-black/40 shadow-soft min-w-[min(860px,90vw)]">
            <div className="relative aspect-video"><img src={img.src} alt={img.alt||''} draggable={false} className="w-full h-full object-cover select-none"/></div>
          </div>))}
        </div>
      </div>
    </div></div></section>);
