...
export default function HUDStrip({ images }:{images:{src:string;alt?:string}[]}){
  ...
  return (
    <section className="py-8 sm:py-16 bg-white/5 hud-mobile">
      <div className="container">
        <div className="flex items-end justify-between">
          <h3 className="text-xl sm:text-2xl lg:text-3xl font-semibold">HUD / telemetry</h3>
          <span className="text-xs sm:text-sm opacity-70">Use mouse wheel or arrows</span>
        </div>
        <div className="relative mt-4 sm:mt-6 -mx-2 sm:-mx-4 lg:mx-0">
          <div className="pointer-events-none absolute inset-y-0 left-0 w-8 sm:w-12 bg-gradient-to-r from-[rgba(12,12,12,1)] to-transparent rounded-r-xl"/>
          <div className="pointer-events-none absolute inset-y-0 right-0 w-8 sm:w-12 bg-gradient-to-l from-[rgba(12,12,12,1)] to-transparent rounded-l-xl"/>
          ...
          <button aria-label="Next" onClick={()=>scrollBy(1)} className="hidden md:flex absolute z-10 right-2 top-1/2 -translate-y-1/2 px-3 py-1 rounded-full bg-white/20">›</button>
          <div ref={scroller} className="overflow-x-auto no-scrollbar px-2 sm:px-4 lg:px-0 touch-pan-x cursor-grab select-none">
            <div className="flex gap-2 sm:gap-4 snap-x snap-mandatory">
              {images.map((img,i)=>(
                <div key={i} className="snap-center shrink-0 rounded-xl sm:rounded-2xl overflow-hidden border border-white/10 bg-black/40 shadow-soft min-w-[min(200px,60vw)] sm:min-w-[min(400px,80vw)] lg:min-w-[min(860px,90vw)]">
